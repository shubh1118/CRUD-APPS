...
if (!GOOGLE_API_KEY) {
  console.error("Missing GOOGLE_API_KEY environment variable. Please get one from aistudio.google.com/app/apikey");
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!imageUrl) {
    return res.status(400).json({ message: 'Image URL is required for AI review generation.' });
  }

  try {
    const genAI = new GoogleGenerativeAI(GOOGLE_API_KEY as string);
    
    const generativeModel = genAI.getGenerativeModel({ model: "gemini-1.5-flash" }); // Changed from "gemini-pro-vision"
    ...
    try {
        ...
        console.warn(`Warning: Could not fetch or convert image from URL: ${imageUrl}. Error: ${fetchError.message}. AI review will be generated based on text description only.`);
    ...
    if (error.status === 429) {
        return res.status(500).json({ message: "AI review generation failed: Too many requests. Please try again later (rate limit)." });
    }
    if (error.status === 403) {
        return res.status(500).json({ message: "AI review generation failed: Permission denied. Ensure your API key is valid and not tied to a project requiring billing." });
    }
